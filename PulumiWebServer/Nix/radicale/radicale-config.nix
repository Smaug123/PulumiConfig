{
  ...
  cfg = config.services.radicale-config;
  filesystem_folder = "/preserve/radicale/data";
in {
  options.services.radicale-config = {
    enable = lib.mkEnableOption "Radicale calendar server";
    domain = lib.mkOption {
      type = lib.types.str;
      example = "example.com";
  ...
  config = lib.mkIf cfg.enable {
    ...
    users.users."radicale".extraGroups = [config.users.groups.keys.name];
