...
module Input =
    ...
    let map<'a, 'b> (f : 'a -> 'b) (x : Input<'a>) : Input<'b> =
        x.Apply f
        |> ofOutput
...
module Output =
    ...
    let map<'a, 'b> (f : 'a -> 'b) (x : 'a Output) : 'b Output =
        x.Apply f
...
type OutputEvaluator<'ret> = abstract Eval<'a> : Output<'a> -> 'ret
type OutputCrate = abstract Apply<'ret> : OutputEvaluator<'ret> -> 'ret
...
[<RequireQualifiedAccess>]
module InputList =
    let ofOutput<'a> (x : 'a seq Output) : 'a InputList =
        InputList.op_Implicit x
...
[<RequireQualifiedAccess>]
module InputUnion =
    let liftLeft<'a, 'b> (x : 'a) : InputUnion<'a, 'b> =
        InputUnion.op_Implicit x

    let liftRight<'a, 'b> (x : 'b) : InputUnion<'a, 'b> =
        InputUnion.op_Implicit x

type OutputComputation () =
    member _.Bind (x : Output<'a>, f : 'a -> Output<'b>) : Output<'b> =
        x.Apply<'b> f
