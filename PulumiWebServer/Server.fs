...
module Server =
    ...
    let rec waitForReady (PrivateKey privateKey as pk) (address : Address) : Output<unit> =
        output {
            ...
            psi.Arguments <- $"root@{address.Get()} -o ConnectTimeout=5 -o IdentityFile={privateKey.FullName} -o StrictHostKeyChecking=off echo hello"
    ...
    let writeNginxConfig (trigger : Output<'a>) (subdomain : string) (DomainName domain) (EmailAddress acmeEmail) (PrivateKey privateKey) (address : Address) : CopyFile * FileInfo =
        let nginx =
            Utils.getEmbeddedResource "nginx.nix"
            |> fun s -> s.Replace("@@DOMAIN@@", $"{subdomain}.{domain}").Replace("@@ACME_EMAIL", acmeEmail)
        ...
        args.Triggers <- InputList.ofOutput<obj> (trigger |> Output.map (unbox<obj> >> Seq.singleton))
    ...
    let writeUserConfig (trigger : Output<'a>) (keys : SshFingerprint seq) (Username username) (PrivateKey privateKey) (address : Address) : CopyFile * FileInfo =
        let userConfig =
            Utils.getEmbeddedResource "userconfig.nix"
            |> fun s ->
                s.Replace("@@AUTHORIZED_KEYS@@", keys |> Seq.map (fun (SshFingerprint r) -> r) |> String.concat "\" \"").Replace("@@USER@@", username)
        let tmpPath = Path.GetTempFileName () |> FileInfo
